package widgets.supportClasses.components.autocomplete
{
	...
	import widgets.supportClasses.skins.PredictiveSearchAutoCompleteSkin;

	// Component bindable events
	[Event (name="select", type="mx.events.FlexEvent")]
	[Event (name="enter", type="mx.events.FlexEvent")]
	[Event (name="clearText", type="mx.events.FlexEvent")]
	...
	public class AutoComplete extends SkinnableComponent
	{
		public function AutoComplete()
		{
			...
			this.setStyle("skinClass", Class(PredictiveSearchAutoCompleteSkin));
			this.addEventListener(MouseEvent.MOUSE_OUT, onMouseOut)
			collection.addEventListener(CollectionEvent.COLLECTION_CHANGE, collectionChange)
		}

		public static const AUTOCOMPLETE_SELECT:String = "autocomplete_select";
		...
		override protected function partAdded(partName:String, instance:Object) : void{
			...
			if (instance==inputTxt){
				...
				inputTxt.addEventListener("clearText", clearButtonHandler)
				inputTxt.text = _text;
				inputTxt.prompt = _prompttext;
		...
		/* Search Prompt Property
		--------------------------------------------------------------- */
		private var _prompttext:String = "";
		public function set prompt(t:String):void
		{
			_prompttext = t;
			if (inputTxt) inputTxt.prompt = t;
		}
		...
		private function onKeyDown(event: KeyboardEvent) : void
		{
			if (popUp.displayPopUp){
				switch (event.keyCode){
					case Keyboard.UP:
					case Keyboard.DOWN:
						var selInd:int = list.selectedIndex;
						var lstLen:int = list.dataProvider.length;
						if (selInd != -1 && lstLen > 0)
						{
							if (event.keyCode == Keyboard.UP)
							{
								// Go to the selected previous object in the list 

								// Check if the top most object is the selected object
								if (selInd == 0)
								{
									// Set to the bottom option
									list.selectedIndex = lstLen - 1;
									list.layout.verticalScrollPosition = 
										list.dataGroup.contentHeight - list.height;
								}
								else
								{
									list.selectedIndex = selInd - 1;
									list.layout.verticalScrollPosition = 
										(list.dataGroup.contentHeight - list.height)/lstLen * selInd;
								}
							}
							else
							{
								// Go to the selected next object in the list 
								
								// Check if the bottom most object is the selected object
								if (selInd == lstLen - 1)
								{
									// Set to the top most option
									list.selectedIndex = 0;
									list.layout.verticalScrollPosition = 0;
								}
								else
								{
									list.selectedIndex = selInd + 1;
									list.layout.verticalScrollPosition = 
										(list.dataGroup.contentHeight - list.height)/lstLen * (selInd + 2);
								}
							}
						}
						else if (selInd == -1 && lstLen > 0)
						{
							// Set the first object in the list box as the selected object
							list.selectedIndex = 0;
							list.layout.verticalScrollPosition = 0;
						}
						else
						{
							// Dispatch the list keypress event
		...
		private function clearButtonHandler(event:FlexEvent):void
		{
			dispatchEvent(event);
		}
		...
		public function acceptCompletion() : void
		{
			if (list.selectedIndex >= 0 && collection.length>0)
			{
				...
				var e:FlexEvent = new FlexEvent("select");
				dispatchEvent(e)
			}
			else {
		...
		private function mouseOutsideHandler(event:Object):void
		{
		...
		private function close(event:Object):void
		{
			popUp.displayPopUp = false;
		}
